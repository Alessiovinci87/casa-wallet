...
function urgency(p) {
  ...
  return { ring: "border-slate-200 bg-white", label: `tra ${p.daysRemaining}g` };
}

function ProductCard({ p, onDismiss, onAlways }) {
  const u = urgency(p);
  return (
    <div className={`rounded-xl p-3 shadow-sm border ${u.ring}`}>
      <div className="flex items-start justify-between gap-2">
        ...
        <span className={`text-xs px-2 py-0.5 rounded-full whitespace-nowrap ${p.isDue ? "bg-rose-600 text-white" : "bg-slate-100 text-slate-500"}`}>
          {u.label}
        </span>
      </div>
      <div className="flex gap-3 mt-2 text-xs">
        <button onClick={() => onDismiss(p.canonicalName)} className="text-slate-400 hover:text-rose-600">Rimuovi</button>
        {!p.isRecurring && (
          <button onClick={() => onAlways(p.canonicalName)} className="text-emerald-600 hover:underline">Compro sempre</button>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <section>
        ...
        {due.length === 0 ? (
          <p className="text-sm text-slate-400">Niente da ricomprare per ora. 🎉</p>
        ) : (
      ...
      <section className="bg-white rounded-xl p-4 shadow-sm">
        ...
        {recurring.length === 0 ? (
          <p className="text-sm text-slate-400">Nessun prodotto ricorrente.</p>
        ) : (
          <div className="divide-y divide-slate-100 text-sm">
            {recurring.map((r) => (
              <div key={r.id} className="py-2 flex items-center justify-between gap-2">
                <span className="capitalize flex-1">{r.canonicalName} {r.alwaysBuy && <span className="text-xs text-emerald-600">(sempre)</span>}</span>
                ...
                <button onClick={() => removeRecurring(r.canonicalName)} className="text-slate-300 hover:text-rose-600" title="Rimuovi">✕</button>
              </div>
            ))}
          </div>
        )}
      ...
      {notPredictable.length > 0 && (
        <section>
          <h2 className="font-semibold mb-2 text-slate-500">Non ancora prevedibili</h2>
          <p className="text-xs text-slate-400 mb-2">Comprati una sola volta: servono almeno 2 acquisti per stimare la frequenza.</p>
          <div className="flex flex-wrap gap-2">
            {notPredictable.map((p) => (
              <span key={p.canonicalName} className="text-xs bg-slate-100 text-slate-600 rounded-full px-3 py-1 capitalize">
                {p.canonicalName}
              </span>
            ))}
          </div>
