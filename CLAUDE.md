...
- Home: con più conti, un riquadro per conto (saldo, +entrate/−uscite del mese, tap → `/movements?tab=expenses&account=<id>`); `GET /transactions?accountId=` filtra per conto (il predefinito include le transazioni senza conto); in Movimenti Segmented "Tutti i conti / …" (param `account` mantenuto tra i tab).
