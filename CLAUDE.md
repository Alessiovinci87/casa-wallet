...
- Home: con più conti, il Disponibile reale sparisce come numero grande: **una card grande per conto** (saldo, +entrate/−uscite del mese, tap → `/movements?tab=expenses&account=<id>`) nell'ordine `BankAccount.sortOrder` (Impostazioni → Conti: ↑ su / ↓ giù, `PUT /api/accounts/reorder {ids}`), sotto una riga "Disponibile reale (totale meno impegni) X ›" che apre il foglio breakdown. Con un solo conto resta il numero grande. `RecurrenceFields`: per le periodiche tendina "Mese della prima scadenza" (sposta la data di inizio al primo mese non passato col giorno scelto); `GET /transactions?accountId=` filtra per conto (il predefinito include le transazioni senza conto); in Movimenti Segmented "Tutti i conti / …" (param `account` mantenuto tra i tab).
