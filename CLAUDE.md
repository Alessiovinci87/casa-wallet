...
- `GET /analytics/spending?from&to&accountId&type` → `{ total, count, fixed, variable, byCategory, byMerchant[{merchant,total,count,category}], byWhat, withoutMerchant{total,count} }` sulle TRANSAZIONI (non sugli scontrini).
