...
- **Sessione "Layout & Revisione" (4 set 2026)** — 3 commit (A layout `e660418`, B revisione `b29090e`, C pronto al push `4748ba0`), ora in prod. Nessuna feature nuova.
  ...
  - **B. Regole confermate/documentate**:
    ...
    - *Prestito interno*: UI in Tesoreria (non in Obiettivi); rifiuto con messaggio "massimo consentito"; dal 4 set 2026 il Disponibile reale **include** (somma) i prestiti aperti: vedi "Fondo tasse iniziale + prestito interno".
...
- **6 nuove funzionalità (14 lug 2026)** — commit `f7ddab9`, live (Railway+Vercel+APK). Testato E2E in locale (stima al centesimo, verify-email flusso completo, alert soglia, generate idempotente)
  ...
  - **Scadenze precompilate**: `POST /api/deadlines/generate {year?}` → crea 30/6 (IRPEF_SALDO) e 30/11 (IRPEF_ACCONTO) dagli importi stimati, idempotente per type+anno; bottone "Crea scadenze da stima"; senza incassi nell'anno precedente → 400 `{ error, code: "NO_HISTORY", noHistory: true }` con l'istruzione di inserirle a mano
...
- `GET /opening` / `PUT /opening { amount|null }` → fondo tasse iniziale ("già accantonato", vedi sezione dedicata)
...
### Conti bancari (`/api/accounts`) — protette, scoped famiglia (4 set 2026)
- Modello `BankAccount` (name, number = numero conto/IBAN per riconoscere gli estratti, openingBalance/openingBalanceDate = punto zero del conto, isDefault) + `Transaction.accountId?` e `RecurringRule.accountId?` (null = conto predefinito). `lib/accounts.js`: `ensureAccounts` crea "Conto principale" dal punto zero della famiglia se non esistono conti (migrazione lazy), `computeAccountBalances` (saldo = Σ conti; il predefinito raccoglie anche le transazioni senza conto; **le transazioni datate nel giorno del punto zero NON si sommano**: il saldo iniziale è il saldo "di adesso", contano da `openingBalanceDate + 1 giorno`; restano nelle statistiche del mese e nei resoconti), `matchAccount` (cifre finali del numero/IBAN), `resolveAccountId`. `Household.openingBalance` resta sincronizzato = Σ punti zero dei conti (`PUT /household/opening-balance` aggiorna il conto predefinito).
- `POST /:id/adjust { balance, note? }` → **Rettifica saldo** (8 set 2026, senza home banking il saldo può non tornare): la differenza tra il saldo reale e quello calcolato diventa una Transaction di oggi con categoria `"Rettifica saldo"` (INCOME/EXPENSE, metodo TRANSFER, `what` = nota), eliminabile dai Movimenti; se il punto zero del conto è oggi o futuro corregge invece `openingBalance` (`mode: "opening"`). Risposta `{ ok, diff, balance, mode: none|transaction|opening, transaction }`. Le rettifiche sono ESCLUSE dal Consulente e da `/analytics/spending` (non sono spese). Client: `lib/adjustBalance.js` (`adjustAccountBalance(account)` → prompt col saldo reale), bottone "Rettifica saldo" in Impostazioni → Conti e "Rettifica" accanto a ogni conto nel foglio breakdown della Home.
- `GET /` → `{ accounts[{…, balance}], balance, hasOpeningBalance }`; `POST /`, `PUT /:id`, `DELETE /:id` (movimenti e regole passano al predefinito; l'ultimo conto non si elimina). Reset famiglia cancella anche i conti.
- `GET /dashboard/available` ora include `accounts[]` (saldo per conto), `periodicAccrued` e `periodicItems[]`: le spese periodiche (bimestrali/semestrali/annuali) NON entrano più in "entro fine mese" ma come **quota maturata** = amount × (P − mesiAllaScadenza) / P (min una quota) → la rata semestrale pesa 1/6 al mese. La previsione riparte da `available + committed + periodicAccrued`.
- `GET /recurring-rules`: ogni regola ha `accrued`, `remainingThisYear {count,total,dates}`, `nextDates[2]`; totali `yearRemainingExpense/Income`.
- Import: la preview ritorna `detectedAccountNumber`, `accountMatched`, `accountId` (conto riconosciuto o predefinito) e `accounts[]`; il commit accetta `accountId`. Transazioni e ricorrenze accettano `accountId` (POST/PUT); `GET /transactions` include `account {id,name}`.
- Home: con più conti, il Disponibile reale sparisce come numero grande: **una card grande per conto** (saldo, +entrate/−uscite del mese, tap → `/movements?tab=expenses&account=<id>`) nell'ordine `BankAccount.sortOrder` (Impostazioni → Conti: ↑ su / ↓ giù, `PUT /api/accounts/reorder {ids}`), sotto una riga "Disponibile reale (totale meno impegni) X ›" che apre il foglio breakdown. Ogni card mostra in rosso la **prossima uscita** del conto (da `GET /recurring-rules/upcoming?days=60`, gli eventi portano `accountId`) e un prospetto comprimibile "Uscite di <mese>: N voci · previste X" = uscite registrate + ricorrenze in arrivo nel mese (stato fatta / in arrivo / da confermare). Sotto la prossima uscita: "saldo previsto il <giorno>" = saldo del conto ± tutte le ricorrenze (entrate e uscite) fino a quel giorno incluso (si ricalcola a ogni cambio di movimenti/ricorrenze via WS). Header (`Layout.jsx`): tasto "‹ Indietro" su tutte le pagine tranne le 5 principali (`navigate(-1)`, fallback `/more`). Altro → "Aggiorna app" (`lib/refresh.js`: cancella Cache Storage, disiscrive i service worker, ricarica con `?v=`). In Movimenti il filtro conto sta in `MovementsPage` e vale per tutte le schede, Ricorrenze inclusa (`RecurringPage` prop `accountId`). Con un solo conto resta il numero grande. `RecurringRule.accrualStart?` ("Accantonamento: parto da oggi"): finché la prossima scadenza è la prima dopo quella data, quota = amount / mesi da accrualStart alla scadenza inclusi, maturata un mese alla volta (`accrualInfo` in `recurrence.js`, esposta come `rule.accrual {accrued, monthlyQuota, monthsTotal, catchUp}` e in `periodicItems`); dopo la prima scadenza torna amount/P. `RecurrenceFields`: per le periodiche tendina "Mese della prima scadenza" (sposta la data di inizio al primo mese non passato col giorno scelto); `GET /transactions?accountId=` filtra per conto (il predefinito include le transazioni senza conto); in Movimenti Segmented "Tutti i conti / …" (param `account` mantenuto tra i tab).
- Client: `store/accountStore.js`, `components/AccountPicker.jsx` (Segmented, nascosto con un solo conto: TransactionForm, RuleForm), `components/AccountsManager.jsx` (Impostazioni → "Conti e saldo iniziale", Punto zero passo Saldo → "Hai più di un conto?"), chip conto nei movimenti e nelle ricorrenze, Home: sottotitolo con saldo per conto + foglio breakdown con conti e accantonato periodico; Ricorrenze: card "Ancora da pagare entro fine anno", per regola "Scadenze 30 dic · 30 giu · 200 €/mese, maturati 600 €" e "Entro fine anno N × importo"; `RecurrenceFields` mostra le prossime scadenze e la quota mensile per le periodiche.


### Tracciamento acquisti: "Dove" e "Cosa" + Nuovo movimento a pagina intera (4 set 2026)
- `Transaction.merchant?` ("Dove": Amazon, Conad, AliExpress…) e `Transaction.what?` ("Cosa": cuffie, spesa settimanale…), indice `[householdId, merchant]`. POST/PUT `/transactions` li accettano; senza `description` la descrizione diventa "Dove · Cosa". Scontrini (`POST /receipts createTransaction`) → merchant = store, what = "N prodotti". Import estratto → `guessMerchant(description)` (lista marchi in `bankImport.js`: Amazon, AliExpress, Temu, Shein, Conad, Lidl, Enel, TIM…).
- `GET /transactions/merchants?type=` → `{ merchants[{merchant, count, total, lastAt, category (moda), method (moda), whats[≤8]}], recentWhats[≤20] }` (ordinati per uso recente) → suggerimenti del form.
- `GET /analytics/spending?from&to&accountId` → `{ expense{actual,planned,total,count,fixed,variable}, income{…}, net, byCategory, byMerchant[{merchant,total,planned,count,category}], byWhat, planned[{date,type,amount,description}], withoutMerchant{total,count} }` sulle TRANSAZIONI + le **ricorrenze del periodo non ancora scattate** ("in arrivo", da domani a `to`, con merchant = descrizione regola). La pagina mostra sempre il confronto Entrate vs Uscite con barre e "Resta / Esce più di quanto entra".
- Client: **`/add?type=expense|income`** (`AddTransactionPage`, a pagina intera, tab bar nascosta, FAB → qui): importo grande con tastiera numerica → Dove (input + chip dei negozi usati, filtrati mentre scrivi; scegliendone uno si precompilano categoria e metodo) → Cosa (chip delle "cosa" del negozio + recenti) → categoria → Quando (Oggi/Ieri/data) → conto + metodo → "Altre opzioni" (% tasse, nota, Si ripete con `RecurrenceFields`) → barra fissa in basso "−23,50 · Amazon · libro" con **Salva** e **+ un'altra** (resta sulla pagina). Entrata con obiettivi → `AllocateModal`. Il vecchio `TransactionForm` resta per la **modifica** (ora con campi Dove/Cosa). Lista movimenti: titolo = "Dove · Cosa" se presenti.
- **`/spending` "Dove vanno i soldi"** (Altro): mese/anno ‹ ›, filtro conto, totale + fisse/variabili, viste Dove / Categoria / Cosa con barre e %, avviso sui movimenti senza "Dove". Voce "Analisi" rinominata "Analisi scontrini".


### Consulente (`/api/advisor`, pagina `/advisor`) — 4 set 2026
- `lib/advisor.js` `buildAdvisorReport({householdId, userId, months})`: capacità mensile = entrate attese (max tra Σ ricorrenze INCOME e media storica, storico pesato con mese corrente parziale) − fisse (Σ monthlyEquivalent EXPENSE) − variabili medie → `free`; `goalsQuota` = Σ (catchUpQuota ?? monthlyQuota); `margin`. Verdetto per obiettivo OK / TIGHT / NO con opzioni (sposta la data a X, tagli proposti liberano Y, servono Z di entrate) e alert (quota del mese non versata dal giorno 20, indietro rispetto al piano). Tagli: voci variabili ("Cosa" → "Dove" → categoria) con quota di taglio per categoria (`CUT_SHARE`, essenziali 10%), `cutsTotal`; abbonamenti piccoli (≤30 €/mese non Casa/Bollette/Tasse). `messages[]` (ok/info/warn/bad). `text` = Markdown esportabile (`reportToText`). Deterministico, niente AI.
- `GET /report?months=1|3|6|12`; `POST /send {force}` invia la notifica (push + email col testo) alla propria famiglia. Cron `30 9 1 1,4,7,10 *` Europe/Rome → `sendQuarterlyReports` a tutte le famiglie con storico.
- Client `AdvisorPage`: periodo, messaggi, capacità, obiettivi con verdetto/opzioni/alert, "Dove tagliare" con spunte e risparmio cumulato, abbonamenti, "Condividi / copia" (Web Share o clipboard: da incollare a un assistente) e "PDF" (`window.print`, CSS `@media print`). Voce in Altro e mini card "Consulente" in Home.

### Restyling "app, non sito" (4 set 2026, sera)
- **Design system in `index.css`**: card bianche senza bordo (ombra appena accennata, angoli 18px), liste raggruppate `.list` + `.row` (riga 56px, separatore rientrato, `.row-icon`, `.row-chevron`), titoli di sezione piccoli maiuscoli grigi (regola globale su `main h2` fuori da card/dialog; `.h-plain` per un h2 normale), campi pieni senza bordo alti 48px con font 16px (niente zoom iOS; regola globale su `main input/select/textarea`, l'importo di Nuovo movimento resta nudo via `input.bg-transparent`), bottoni `.btn .btn-primary/.btn-secondary/.btn-danger/.btn-ghost` (48px, angoli 14px) e feedback al tocco `scale(.97)` su ogni bottone, chip `.chip/.chip-active`, controllo segmentato `.seg/.seg-item/.seg-active`, `.amount-hero` (44px) per il numero grande, `.action-bar` per la barra Salva. Font di sistema (SF Pro / Segoe / Roboto).
- **Fondo verde Awareness** (`--ground: #0a6847`, richiesto da Alessio dopo il primo deploy): guscio, header e pagine pubbliche verdi, card/liste bianche. Fuori dalle card i colori si ribaltano via regole globali in `index.css` con `:not(.card):not(.card *)…` (titoli e testi bianchi/traslucidi, `.seg` e `.chip` bianco/20 con attivo bianco e testo verde, `.btn-primary`/`bg-brand-600` bianchi, `text-rose/tax` schiariti, FAB bianco). Attenzione: una classe colore messa sull'elemento stesso `.card` va esclusa esplicitamente (`:not(.card)`), non basta `:not(.card *)`. Tab bar bianca. `apple-mobile-web-app-status-bar-style` = black-translucent.
- **`Segmented`**: fino a 4 opzioni corte → segmentato stile iOS (binario grigio, attivo bianco); altrimenti chip a capo (prop `chips` per forzare).
- **`components/Sheet.jsx`**: foglio dal basso (maniglia, titolo, X, Esc, safe-area; su desktop finestra centrata). **`lib/dialog.js` + `components/DialogHost.jsx`** (montato in Layout): `await dialog.confirm({title?, message, okLabel?, danger?})` → bool, `dialog.prompt({..., defaultValue?, inputMode?, placeholder?})` → string|null, `dialog.alert(...)`, `askAmount(...)`. **Tutti i `window.prompt/confirm/alert` sono stati sostituiti** (Goals, Recurring, Transactions, Treasury, Settings, Invoices, Import, AccountsManager, More): mai più dialoghi nativi (brutti su iOS, inaffidabili in webview). Anche `.modal-backdrop/.modal-panel` (TransactionForm, RuleForm…) ora si aprono dal basso con la stessa animazione.
- **Pagine**: Altro = menù raggruppato (riga profilo → Impostazioni; Registra / Analizza / Partita IVA / App con icone; Guida, Aggiorna, Ricomincia, Esci); Tesoreria a **3 schede** (Scadenze · Fondo · Profilo) con le scadenze come righe compatte (tap → foglio con Segna pagata / Modifica / Elimina) e lo scope Solo io/Famiglia nella scheda Fondo; Home: hero `card` + 4 mini card in griglia 2×2 (niente scroll orizzontale), banner email e notifiche compatti; Nuovo movimento: chip dei negozi **sopra** il campo Dove, categoria compatta "come le altre volte da X · Cambia" quando è suggerita, barra Salva `.btn`; Punto zero: barra di avanzamento non cliccabile. Header senza bordo, titolo 22px; tab attiva con icona più spessa. Nuove icone in `Icons.jsx` (Wallet, Receipt, Pie, Cart, Gauge, Upload, Settings, Spark, Coin, Repeat, Check, ChevronRight).
- **Indietro** = pagina genitore fissa (`PARENTS` in `Layout.jsx`), non `history(-1)`.
- Verifica: `scratchpad/pw_all.mjs` su 16 route + schede Tesoreria + foglio scadenza + prompt Versa a 390 px: 0 errori JS, nessun overflow, nessun testo < 13 px, tap target ≥ 44 px (unico residuo: un link inline "profilo fiscale" in Fatture). API: 55/55.

### Guscio app senza barre fisse (4 set 2026, sera)
- Problema segnalato: su iPhone le pagine lunghe "si rompevano" (barre `position: fixed` che saltano con la toolbar di Safari e con la tastiera, contenuto coperto dalla tab bar). Ora `Layout.jsx` è un **guscio**: `.app-shell` alto quanto il viewport visuale (`--vvh` da `visualViewport`, fallback `100dvh`), tre righe: header (in flusso, `pt-[env(safe-area-inset-top)]`) · `<main class="app-main">` unico elemento scorrevole · tab bar in flusso sotto (mai sopra il contenuto). `html/body/#root` a 100% con `body { overflow: hidden }`; login/registrazione/spinner usano `.page-scroll`. Il FAB è `absolute` dentro l'area di main; niente `position: fixed` nella navigazione. A ogni cambio route `main.scrollTo(0,0)`. Con la tastiera aperta: tab bar e FAB spariscono, il guscio si restringe a `--vvh` e `window.scrollTo(0,0)` riporta il layout viewport (iOS lo sposta).
- `/add`: la barra "Salva / + un'altra" è `sticky bottom-0` dentro main (visibile anche con tastiera aperta, non copre l'ultima card). **FAB → `/add?type=expense` in un tocco** (importo già a fuoco; in pagina Segmented Uscita/Entrata e link scontrino); il foglio "Nuovo" con le 3 voci resta solo per il bottone della sidebar desktop.
- Verifica: `scratchpad/pw_shell.mjs` (31 check a 390×844 e 1280): body non scorre, tab bar in fondo, FAB sopra la tab bar, scorrendo in fondo l'ultimo elemento non è coperto, Salva sticky, viewport ridotto ok, foglio breakdown ok. **Da provare su iPhone reale** (Safari e "Aggiungi a Home"): se la tab bar dà ancora fastidio, il piano B è spostare le 5 voci nell'hamburger (Altro) — tutto in `Layout.jsx`.

### Fondo tasse iniziale + prestito interno: regole del piano di rientro (4 set 2026)
- **"Già accantonato per le tasse"**: `TaxSaving.transactionId` ora nullable + `userId?` + `note?`. Il fondo iniziale è UN record personale senza transazione (`note: "saldo iniziale"`, pending). `GET/PUT /api/tax-savings/opening { amount|null }` (null/0 → il record sparisce, reversibile). Campo nel Punto zero (passo tasse, con P.IVA) e in Tesoreria (card "Fondo tasse" sopra il simulatore). Il proprietario di un accantonamento è SEMPRE `transaction.userId` oppure `userId`: usare `ownTaxSavingWhere(userId)` / `householdTaxSavingWhere(householdId)` / `pendingTaxFund(userId)` di `lib/taxFund.js`, mai il filtro `transaction: {userId}` da solo. Entra in `totalPending`, nel Disponibile reale, nel simulatore e nella previsione.
- **Fallback dichiarativo** (`buildDeclaredCapacity` in `treasury.js`): sotto 3 mesi di storico (`GET /profile` resta `DATI_INSUFFICIENTI`, i mesi importati da CSV contano) il simulatore NON si ferma: capacità = ricorrenze INCOME mensilizzate − ricorrenze EXPENSE mensilizzate − quote obiettivi del mese − Σ CategoryBudget; incassi attesi (fatture EMESSE) come somme una tantum. Scenari: pessimista 75% senza incassi, realistico −buffer, ottimista pieno. La risposta porta `basis: "storico"|"dichiarato"`, `basisLabel`, `declared {income, fixed, goalsQuota, budgets, capacity}`, `missing[]` (cosa manca: entrate dichiarate, budget). Mai un NO muto: il rifiuto del prestito riporta il motivo e `missing`.
- **Piano di rientro** (`lib/repaymentPlan.js`): rate = importo ÷ **mesi pieni** tra oggi e la prossima `TaxDeadline` non pagata (4/9 → 30/11 = ottobre e novembre = 2 rate da 500 su 1.000), nel giorno del mese del prelievo, mai oltre la scadenza, min 1 rata (stesso mese → tutto alla scadenza). Il simulatore ritorna `repaymentPlan {count, installment, installments[{date, amount}], dueDate, fundAfter}`; il prestito espone `installments`, `remainingInstallments` (le prime si considerano coperte da `repaid`), `nextInstallment`; `POST /loans` risponde anche `fundAfter`. `expectedRepaidByNow` = Σ rate con data ≤ oggi (LATE se `repaid` sotto, tolleranza 2%).
- **Guardrail**: verdetto NO → 400 `VERDICT` sempre; RISCHIO → 400 `VERDICT_RISK` salvo `force: true` (conferma esplicita in UI, salvata in `InternalLoan.forced`); poi cap `maxSelfFinancePercent` × fondo − prestiti aperti → 400 `CAP` con massimo consentito. Il verdetto si controlla PRIMA del cap.
- **Effetti**: il Disponibile reale **sale** del prestito aperto (`available = saldo − (taxPending − loansOutstanding) − …`, voce breakdown "Prelevati dal fondo tasse (da rientrare)" con segno +): i soldi sono liberi, il rientro compare in Previsione come eventi `kind: "loan"` alle date delle rate residue; la scadenza fiscale in Previsione è coperta dal fondo **al netto dei prestiti aperti più le rate che la precedono**. `GET /loans` ha `fundFree` = fondo − prestati.
- Test: `scratchpad/e2e_loan.mjs` (55 check, DB `server/prisma/test_loan.db` su porta 3021, ricreabile) + `late_check.mjs` (LATE) + Playwright `pw_treasury.mjs` a 390 px: nessun errore JS, nessun overflow, nessun testo < 13 px. Scenario di riferimento: fondo 4.000, scadenza 30/11 da 2.500, prestito 1.000 → verdetto OK (storico 6 mesi da CSV; dichiarato con entrata 3.200 + budget 500), 2 rate da 500 il 4/10 e 4/11, restano 3.000 nel fondo.

### Prestiti interni (`/api/loans`) — protette, PERSONALI
- `GET /?includeClosed=` → `{ loans[...outstanding, expectedRepaidByNow, behindBy, progress, daysToDue, suggestedRepayment, installments, remainingInstallments, nextInstallment, forced], outstanding, openCount, fundAvailable, fundFree, maxPercent, cap }`
- `POST / { amount, note?, scope?, force? }` → 201 `{ loan, simulation, cap, maxPercent, fundAfter }` oppure 400 `{ error, code: VERDICT|VERDICT_RISK|CAP|NO_DEADLINE, cap?, maxPercent? }` (force accetta solo RISCHIO)
...
- **Formati** (`lib/statementParsers.js`, rilevati dal contenuto): CSV; Excel xls/xlsx e Excel-XML (SheetJS, foglio più lungo, preambolo tagliato da `trimToTable`); XML ISO 20022 camt.052/053 (→ colonne fisse Data/Descrizione/Importo, `autoMapping`); XML CBI/tabellare generico (chiavi → intestazioni, colonna "Importo con segno" da Segno D/C); PDF (pdf.js con coordinate: righe per Y, `mergeGlyphs` unisce i PDF con un frammento per lettera, segno da segno esplicito → saldo progressivo → colonna Dare/Avere dell'intestazione più vicina in X, descrizione prima o dopo gli importi, continuazioni per colonna X, righe SALDO/riepilogo scalare scartate; `autoMapping`; PDF scansionati → 400). Excel/CSV: `guessMapping` propone il mapping dalle intestazioni (Data operazione, Descrizione, Entrate/Uscite). `cleanDescription` (in `bankImport.js`) pulisce le code tecniche per tutti i formati, così PDF ed Excel dello stesso movimento hanno lo stesso hash. Date italiane "03 settembre 2026" supportate. **Verificato sui 4 file reali di Banco di Sardegna** (elenco movimenti PDF e XLS, estratti trimestrali "E/C scalari" PDF di 2 conti): totali entrate/uscite al centesimo. La response ha `format` e `autoMapped` (true → il client salta la mappa colonne e il mapping CSV salvato non viene toccato). Limite file 15 MB.
